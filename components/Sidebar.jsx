...
import React, { useState } from "react";
import Link from "next/link";
// Import your custom components here (Icon, DrawerLink, CollapsibleMenu, MenuSectionTitle, etc.)
// import { Icon, DrawerLink, CollapsibleMenu, DrawerSublink, MenuSectionTitle } from "@/components/..."
...
const SUPERADMIN_MENU = [
  {
    ...
    items: [
      { label: "Organizations", href: "/organization", icon: "facility" },
      { label: "Score Management", href: "/score-management", icon: "activity" },
      {
        label: "Role Management",
        icon: "users", // using string icons for your custom <Icon />
        key: "role-management",
        children: [
          { label: "Role List", href: "/role" },
          // { label: "Add Role", href: "/role-management/add" },
        ],
      },
      {
        label: "All Users",
        ...
        key: "all-users",
        children: [
      { label: "Superadmin", href: "/user/superadmin" },
          { label: "Admin", href: "/user/admin" },
          { label: "Supervisor", href: "/user/supervisor" },
          { label: "Cleaner", href: "/user/cleaner" },
        ],
      },
    ],
  },
...
const CLIENT_MENU = [
    ...
    items: [
      {
        ...
        key: "location",
        children: [
          { label: "View Hierarchy", href: "/locationHierarchy" },
          { label: "Add Hierarchy", href: "/locationHierarchy/add" },
        ],
      },
      {
        label: "Washrooms",
        ...
        badge: "53",
        children: [
          { label: "Washroom List", href: "/washroom" },
          { label: "Add Washroom", href: "/add_washroom" },
        ],
      },
      {
        ...
        badge: "30",
        children: [
          { label: "User List", href: "/userMangement" },
          { label: "Add User", href: "/userMangement/add" },
        ],
      },
      {
        label: "User Mapping",
        ...
        key: "mapping",
        children: [
          { label: "Mapped List", href: "/userMaping" },
        ],
      },
      {
        ...
        key: "facility",
        children: [
          { label: "View List", href: "/facility_list" },
          { label: "Add Facility", href: "/add_facility" },
        ],
      },
    ],
  },
  {
    title: "Operations",
    items: [
      { label: "Locate On Map", href: "/map", icon: "pin" },
      { label: "Cleaner Activity", href: "/cleaner-activity", icon: "activity" },
  ...
  },
  {
    title: "System",
    items: [
      { label: "Dynamic Configuration", href: "/dynamic-configuration", icon: "settings" },
      { label: "User Review", href: "/reviews", icon: "chat" },
...
export default function Sidebar({ isOpen, onClose, activePath }) {
  // Toggle between "main" (Superadmin) and "client" (Company Admin)
  ...
  // Select which array to render based on state
  const currentMenu = dashboardMode === "main" ? SUPERADMIN_MENU : CLIENT_MENU;

  return (
    <>
      {/* Scrim Overlay */}
      <div
        className={`absolute inset-0 bg-[rgba(10,25,20,.4)] backdrop-blur-[2px] transition-opacity duration-250 z-[60] ${
      ...
      {/* Drawer */}
      <aside
        className={`absolute top-0 bottom-0 left-0 w-[82%] max-w-[320px] bg-gradient-to-br from-white to-safai-surface-tint z-[61] p-[22px] pt-[22px] flex flex-col overflow-hidden transition-transform duration-300 ease-[cubic-bezier(.2,.9,.3,1)] shadow-[12px_0_40px_rgba(10,25,20,.18)] ${
      ...
      >
        ...
        <div className="flex items-center gap-3 pb-[18px] mb-4 border-b border-safai-border cursor-pointer rounded-[14px] -mx-1.5 px-1.5 pt-2.5 active:bg-safai-surface-soft transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[14.5px] font-bold font-display">Omkar Porlikar</div>
            <div className="text-[10px] font-extrabold text-safai-primary-deep tracking-[.04em] mt-0.5">
              SUPER ADMIN
            </div>
          </div>
          <Icon name="chevRight" className="w-[14px] h-[14px] text-safai-text-faint shrink-0" />
        ...
        <div className="flex-1 overflow-y-auto -mx-[18px] px-[18px] hide-scrollbar">
          ...
          {/* STATIC DASHBOARD TOGGLES */}
          <MenuSectionTitle title="Overview" />
          
          {/* Clicking this sets the menu to SUPERADMIN */}
          <div onClick={() => setDashboardMode("main")}>
            <DrawerLink
              icon="grid"
              label="Main Dashboard"
              href="/"
              active={dashboardMode === "main"}
            />
          </div>

          {/* Clicking this sets the menu to CLIENT */}
          <div onClick={() => setDashboardMode("client")}>
            <DrawerLink
              icon="layout"
              label="Client Dashboard"
              href="/" // Or update to `/clientDashboard/${companyId}` if needed
              active={dashboardMode === "client"}
            />
          </div>
